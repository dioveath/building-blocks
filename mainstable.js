window.onload = function(){
  ...
  function renderGame(){
    ...
    context.fillText("HIGHEST SCORE", width/3, -200);
    if(typeof(Storage) != undefined){
      if(localStorage.highestScore){
        context.fillText("" + localStorage.highestScore, width/3, -150);
      } else {
        context.fillText(0000 + "" , width/3, -150);
      }
    } else {
      context.fillText(0000 + "" , width/3, -150);
    }
    context.fillText("SCORE: " + currentScore, width/3, -60);
    context.font = "20px verdana"
    context.fillText("Next Block", width/3 , -20);
    var centerBlock = nextBlock.blocks[0];
    for(var i = 0;i < nextBlock.blocks.length; i++){
      var block = nextBlock.blocks[i],
          x = centerBlock.xIndex - block.xIndex,
          y = centerBlock.yIndex - block.yIndex;
      context.fillRect( (width/3) + (x * GRID_SIZE), 20 + y * GRID_SIZE, GRID_SIZE, GRID_SIZE);
    }
  ...
  function renderGameOver(){
    if(typeof(Storage) != undefined){
      if(localStorage.highestScore){
        if(localStorage.highestScore < currentScore){
      ...
      alert("Sorry, We could not save your score");
    }
    context.font = "30px Verdana";
    context.textBaseline = "middle";
    context.textAlign = "center";
